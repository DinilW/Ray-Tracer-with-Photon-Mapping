 ...
 */
  ...
  *
  * Execute the code using project
  *
...
void refraction(Hit& best_hit, Ray ray, Ray& refracRay, float& refCoef, float& refracCoef)
{
    ...
    {
        //reverse ratio if leaving object
...
void findphotons(Ray ray, Object* objects, Light* lights, Colour& colour, int depth, KD::Tree<CORE>& kdtree, KD::Tree<CORE>& kdtree_caustic)
{
    ...
    if (best_hit.flag)
    {
        ...
        while (light != (Light*)0)
        {
            ...
            //if shadow photons are present or there are no direct photons, a shadow ray is tried
...
{
    float x, y, z, scale, correctx, correcty, correctz;
    Ray photonRay;
    float ne = 0; //number of photons
    Hit best_hit;
    Light* light = lights;
...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    emit_photons_caustic(sphere, dl, kdtree_caustic, 20000);
